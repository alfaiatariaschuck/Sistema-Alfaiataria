...
export default function Shell() {
  ...
  const receitaMesAlfaiataria = useMemo(
    () =>
      (pecas || [])
        .filter((p) => !TIPOS_SAIDA_SEM_VENDA.includes(p.tipoSaida) && p.dataPedido && p.dataPedido.slice(0, 7) === mesAtualStr)
          ...
          {loading ? (
            ...
            <>
              ...
              {tab === "dre" && !loadingPecas && !loading && (
                <DRE pedidos={pedidos} pecas={pecas} despesas={despesas} equipe={equipe} custoAviamentosPorPecaBase={custoPorPecaBase} />
              )}
              ...
              {tab === "vendedor" && !loading && (
                <VendedorGestao
                  ...
                  atualizarValorTotalDespesa={atualizarValorTotalDespesa}
                />
              )}
